    ...
    <Card style={{ marginLeft: "8px", marginTop: '1rem', position: "relative", textOverflow: 'ellipsis', overflow: 'hidden' }} sx={{ maxWidth: 250, height: 'min-content', opacity: isDragging ? 0.5 : 1 }} ref={drag}>
      <CardContent style={{textOverflow: 'ellipsis', overflow: 'hidden'}}>
        ...
        <CopyToClipboard text={link}>
          <Button variant="text" size="medium" sx={{ p: '0', textTransform: 'none', color: 'text.secondary' }}>
            Copy Link 🔗
          </Button>
        </CopyToClipboard>
      </CardContent>
      ...
      <CardActions style={{
        ...
        <Button size="medium" sx={{ color: green[500] }} onClick={handleClick}>Watch</Button>
        <Button size="small" sx={{ color: blue[500] }} onClick={handleEdit} > <EditIcon /></Button>
        <Button onClick={() => {
          disPatch(deleteCard({ bucketIndex, cardIndex }))
          rerenderOnce(true)
        }} size="small" sx={{ color: red[500] }} > <DeleteIcon /> </Button>
      </CardActions>
    </Card>
  );
}
